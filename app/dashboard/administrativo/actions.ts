...
export async function eliminarAdministrativo(id: string, email: string) {
  ...
  try {
    ...
    if (callerProfileError) {
      console.error(
        "Error al obtener perfil del usuario que realiza la acción:",
        callerProfileError
      );
    ...
    if (!callerProfile || callerProfile.role !== "super") {
      return {
        success: false,
        error:
          "Permisos insuficientes: sólo usuarios con rol super pueden eliminar administrativos",
      };
    }
    ...
    return {
      success: false,
      error: error.message || "Error al eliminar administrativo",
    };
  }
}
...
export async function actualizarAdministrativo(
  ...
  try {
    ...
    if (auditError) {
      console.warn(
        "Error al registrar auditoría (tabla puede no existir):",
        auditError
      );
    }
  } catch (auditErr) {
...
export async function eliminarAlumno(id: string, email: string) {
  const supabase = await createClient();
  ...
  try {
    ...
    // 2️⃣ Verificar que el usuario tenga rol 'super' o 'admin'
    const { data: callerProfile, error: callerProfileError } = await supabase
      .from("profiles")
      .select("role")
      .eq("id", userSession.user.id)
    ...
    if (!role || (role !== "super" && role !== "admin")) {
      return {
        success: false,
        error:
          "Permisos insuficientes: sólo usuarios con rol super o admin pueden eliminar alumnos",
      };
    }

    // 3️⃣ Eliminar de la tabla usuarios
    ...
    if (usuarioError) {
      throw new Error(`Error al eliminar alumno: ${usuarioError.message}`);
    }

    // 4️⃣ Eliminar de la tabla Roles
    const { error: rolesError } = await supabase
      .from("Roles")
      .delete()
      .eq("email", email);
    ...
    // 5️⃣ Buscar y eliminar el usuario en auth.users
    try {
      const authResponse = await fetch(
        `${process.env.NEXT_PUBLIC_SUPABASE_URL}/auth/v1/admin/users`,
        {
    ...
    // 6️⃣ Eliminar de la tabla profiles
    const { error: profileError } = await supabase
      .from("profiles")
      .delete()
      .eq("email", email);
    ...
    // 7️⃣ Revalidar la ruta para actualizar la vista
